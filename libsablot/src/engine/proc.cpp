...
eFlag OutputDocumentList::finish(Sit S)
{
...
eFlag Processor::createOutputDocument(Sit S, Str& href,
				      OutputDocument *&resultDoc, 
				      OutputDefinition *def)
{

  //OutputterObj *newOut;
  //E( createOutputterForURI(S, href, base, newOut, def) );
...
eFlag Processor::startDocument(Sit S, OutputDocument *doc)
{
  ...
  case OUTDOC_NEW:
      ...
      if (current && (phys = current->getPhysical()) && phys->getDataLine())
	{
      ...
      if (base == "" || base == "file://stdout" || base == "file://stderr")
	{
	  DStr cwd;
	  my_getcwd(cwd);
	  base = "file:";
      ...
      OutputterObj *newOut;
      E( createOutputterForURI(S, href, base, newOut, doc->getDefinition()) );
  ...
  case OUTDOC_FINISHED:
    assert(! "Couldn't write the document twice"); break;
  }
  return OK;
}
...
eFlag Processor::finishDocument(Sit S, OutputDocument *doc, Bool canClose)
{
  ...
  case OUTDOC_NEW:
  case OUTDOC_FINISHED:
    assert(!"Could not finish unopened/finished document"); break;
  case OUTDOC_ACTIVE:
    {
      if (canClose)
	{
	  E( outputter() -> eventTrailingNewline(S) );
	  E( outputter() -> eventEndOutput(S) );
	  doc -> setState(OUTDOC_FINISHED);
	}
      E( popOutputterNoFree(S) );
    }; break;
  }
  return OK;
}
				     ...
				     OutputDefinition *outDef /*=NULL*/)
{
  ...
  E( createOutputterForURI(S, location, base, newOut, outDef) );
...
{
  assert(documentDefinitions.number());
