  ...
  if (state == OUTDOC_ACTIVE)
    {
      ...
      E( NZ(outputter) -> eventEndOutput(S) );
...
{
  assert(!(physical && mySAXOutputType == SAXOUTPUT_INT_PHYSICAL));
  Str physName;
  //  if (S.getProcessor())
				   ...
				   Bool hidden)
{
    ...
    */
	...
	// the following condition was asserted here for some reason
