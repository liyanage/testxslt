...
*/
	...
	assert(name && match && use && match -> expr && use -> expr);
	EQName ename;
	QName q;
...
{
    assert(op == XSL_APPLY_TEMPLATES || op == XSL_FOR_EACH);
					     ...
					     Bool resolvingGlobals)
{
  ...
  OutputDocument *doc;
  E( proc -> createOutputDocument(S, href, doc, def.keep()) );
