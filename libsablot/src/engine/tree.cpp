	...
	case XSL_ATTRIBUTE_SET:
	  {
	    ...
	    for (int i = 0; i < x -> contents.number(); i++)
	      {
		assert(isXSLElement(x -> contents[i]) && 
...
eFlag Tree::insertRule(Sit S, XSLElement *tmpl)
{
    ...
    if (q.getLocal() != UNDEF_PHRASE && 
        subtrees.getCurrent() -> getStructure() -> rules().findByName(
	    *this, q))
    {
	Str fullName;
    ...
    subtrees.getCurrent() -> getStructure() -> rules().insert(
	new RuleItem(tmpl,prio,q,mode.keep()));
    return OK;
}
