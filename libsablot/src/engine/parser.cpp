    ...
    if (itsXSL) 
        ...
        if (opCode == XSL_NONE)
	  {
	    if (this_ -> theDataLine) {
	      if (this_ -> inSAXForAWhile)
		this_ -> S.setCurrFile(this_ -> theSAXUri);
	      else
    ...
    const char* systemId, const char* publicId)
{
    ...
    //PH
    if ( publicId && ! this_ -> S.hasFlag(SAB_PARSE_PUBLIC_ENTITIES)) 
      return 1;
